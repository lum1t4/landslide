...
def compute_mean_and_std(path):
    ...
    pixel_squared_sum = torch.zeros(num_channels)
    ...
    print(
        f"On landslide patches, there are {pixel_landslide_patch_count_pos} positive pixels out of {pixel_landslide_patch_count} total pixels. ({100 * pixel_landslide_patch_count_pos / pixel_landslide_patch_count:.1f}%)"
    )
    ...
    print(
        f"num landslide patches: {patch_count_pos}/{n_patch} ({100 * patch_count_pos / n_patch:.1f}%)"
    )
    print("=" * 50 + "\n")
    return num_classes, mean, std, pixel_pos_weight
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Prepare data for landslide detection model training."
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="dataset/processed/A19",
        help="Path to the dataset YAML file.",
    )
    parser.add_argument(
        "--batch", type=int, default=32, help="Batch size for data loader."
    )
