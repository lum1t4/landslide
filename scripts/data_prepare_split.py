...
def main(src: Path, dest: Path):
    ...
    assert len(im_files) == len(train_files) + len(
        valid_files
    ), "Number of images do not match."

    im_files = [(im_file, "train") for im_file in train_files] + [
        (im_file, "valid") for im_file in valid_files
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--src",
        type=str,
        default=f"./dataset{os.sep}raw{os.sep}A19",
        help="Source directory",
    )
    parser.add_argument(
        "--dst",
        type=str,
        default=f"./dataset{os.sep}interim{os.sep}A19",
